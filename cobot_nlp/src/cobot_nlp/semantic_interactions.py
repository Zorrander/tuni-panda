...
class SemanticInterpreter():
    ...
    def check_action(self, object_symbol):
        template = self.query_engine.load_template('describe_command_results.rq')
        query = self.query_engine.generate(template, "'"+object_symbol+"'")
        print(query)
    ...
    def new_command(self, action, target):
        ...
        target_sem = self.check_manipulation_preconditions(target.lower())
